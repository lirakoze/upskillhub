...
namespace TechTreeMVCWebApplication.Controllers
{
    public class HomeController : Controller
    {
        ...
        [AllowAnonymous]
        public async Task<IActionResult> DownloadCertificateAsync(string courseName, DateTime completionDate)
        {
            // Create a new instance of the CertificateTemplateGenerator
            var certificateGenerator = new CertificateTemplateGenerator();

            var user = await _userManager.GetUserAsync(User);
            var fullnames = $"{user.FirstName} {user.LastName}";
            

            // Generate the certificate content using the provided details
            var certificateContent = certificateGenerator.GenerateCertificateContent(courseName, completionDate,fullnames);

            // Set the content type and return the generated PDF file for download
